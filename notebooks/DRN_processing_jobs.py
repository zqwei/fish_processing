...
def local_pca():
    ...
    for index, row in dat_xls_file.iterrows():
        ...
        save_folder = dat_folder + f'{folder}/{fish}/Data'
        print(f'checking file {folder}/{fish}')
        if not os.path.isfile(save_folder+'/Y_2dsvd.npy') and not os.path.isfile(save_folder+'/proc_local_denoise.tmp'):
            if os.path.isfile(save_folder+'/Y_d.npy') or os.path.isfile(save_folder+'/Data/finished_detrend.tmp'):
                Path(save_folder+'/proc_local_denoise.tmp').touch()
                Y_d = np.load(f'{save_folder}/Y_d.npy').astype('float32')
                denose_2dsvd(Y_d, save_folder)
                Y_d = None
                clear_variables(Y_d)
                get_process_memory();
