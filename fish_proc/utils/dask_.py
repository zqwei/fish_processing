...
def get_local_cluster(dask_tmp=None, memory_limit='auto'):
    ...
    n_workers = cpu_count
    if dask_tmp is None:
        return LocalCluster(n_workers=n_workers, processes=True, threads_per_worker=1, memory_limit=memory_limit)
    else:
        return LocalCluster(n_workers=n_workers, processes=True, threads_per_worker=1, local_dir=dask_tmp, memory_limit=memory_limit)
