...
def imrank_dll(image,r):
    ...
    if sys.platform == 'darwin':  # Mac
        imfc = ctypes.cdll.LoadLibrary(pwd + "/imfc.cpython-36m-darwin.so")
    elif sys.platform == 'linux':
        imfc = ctypes.cdll.LoadLibrary(pwd + "/imfc.cpython-36m-x86_64-linux-gnu.so")
    else:  # Windows
        imfc = ctypes.cdll.LoadLibrary(pwd + "/imfc.dll")
...
def cell_images(imlist,inds):
    ...
    for b in range(block_num):
        ...
        p1=np.empty((image_size*10,image_size*10,3))
        p1[:] = np.nan
        proof_size=min(block_size,len(inds)-(block_size*b))
        ...
        for i in range(proof_size):
            ...
            y=plot_position[1]*image_size
            p1[x:(x+image_size),y:(y+image_size),:]=tmp
        p1 = p1[~np.all(np.isnan(p1[:, :, 0]), axis=1), :, :]
        p1 = p1[:,~np.all(np.isnan(p1[:, :, 0]), axis=0), :]
